...
// Add this function at the end of your combined.js file
function nukeAllBadgesFromOrbit() {
    console.log("NUKING ALL BADGES FROM ORBIT");
    
    // Function to add directly to showSendScreen and showReceiveScreen
    function removeBadgesCompletely(screenId) {
        // Get the screen
        const screen = document.getElementById(screenId);
        if (!screen) return;
        
        console.log(`Removing all badges from ${screenId}`);
        
        // 1. First, find and remove ALL elements that could be badges
        const potentialBadges = [
            ...screen.querySelectorAll('.chain-badge'),
            ...screen.querySelectorAll('.chain-badge-fixed'),
            ...screen.querySelectorAll('[class*="badge"]'),
            ...screen.querySelectorAll('.token-icon > div'),
            ...screen.querySelectorAll('.token-icon-large > div'),
            ...screen.querySelectorAll('.token-selection div > div')
        ];
        
        console.log(`Found ${potentialBadges.length} potential badges`);
        
        potentialBadges.forEach(badge => {
            if (badge.parentNode) {
                badge.parentNode.removeChild(badge);
                console.log("Removed badge:", badge);
            }
        });
        
        // 2. Modify token containers to make it impossible for badges to appear
        const containers = [
            ...screen.querySelectorAll('.token-icon'),
            ...screen.querySelectorAll('.token-icon-large'),
            ...screen.querySelectorAll('.token-selection > div:first-child')
        ];
        
        containers.forEach(container => {
            // Force inline styles with !important
            container.style.cssText += "overflow: hidden !important; position: relative !important;";
            
            // Keep only the img, remove everything else
            ...
            if (img) {
                ...
                container.appendChild(newImg); // Add back only the image
            }
        });
    }
    ...
    window.showSendScreen = function(tokenId) {
        ...
        if (originalShowSendScreen) originalShowSendScreen(tokenId);
        
        // Then nuke the badges
        setTimeout(() => removeBadgesCompletely('send-screen'), 0);
        setTimeout(() => removeBadgesCompletely('send-screen'), 100);
    };
    
    const originalShowReceiveScreen = window.showReceiveScreen;
    window.showReceiveScreen = function(tokenId) {
        // Call original function first
        if (originalShowReceiveScreen) originalShowReceiveScreen(tokenId);
        
        // Then nuke the badges
        setTimeout(() => removeBadgesCompletely('receive-screen'), 0);
        setTimeout(() => removeBadgesCompletely('receive-screen'), 100);
    };
    
    // Also apply immediately to any currently visible screens
    if (document.getElementById('send-screen') && 
        getComputedStyle(document.getElementById('send-screen')).display !== 'none') {
        removeBadgesCompletely('send-screen');
    }
    
    if (document.getElementById('receive-screen') && 
        getComputedStyle(document.getElementById('receive-screen')).display !== 'none') {
        removeBadgesCompletely('receive-screen');
    }
}

// Run this immediately and also on document ready
nukeAllBadgesFromOrbit();

document.addEventListener('DOMContentLoaded', function() {
    // Your existing DOMContentLoaded code
    
    // Run the badge nuking function at the end
    nukeAllBadgesFromOrbit();
});
