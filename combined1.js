...
(function() {
  ...
  console.log('TrustWallet UI Fixes: Starting...');
  ...
  // Fix Header Height for multiple screens
  function fixHeaderHeight() {
    const headers = document.querySelectorAll('#send-screen .screen-header, #receive-screen .screen-header, #history-screen .screen-header');
  ...
  // Create receive screen with tokens list like send screen
  function fixReceiveScreen() {
    const receiveScreen = document.getElementById('receive-screen');
    if (!receiveScreen) return;
    
    // Create receive token selection screen
    ...
    // Create QR Screen as separate screen
    const appContainer = document.querySelector('.app-container');
    if (appContainer) {
      ...
      if (!qrScreen) {
        ...
        qrScreen.className = 'screen hidden';
        qrScreen.innerHTML = `
          <div class="screen-header">
            <button class="back-button" aria-label="Go back">
              <i class="fas fa-arrow-left"></i>
            </button>
            <h2>Receive <span id="receive-token-symbol">BTC</span></h2>
            <div class="placeholder-icon"></div>
          </div>
          ...
          <div class="receive-content">
            ...
            <div class="wallet-address-container">
              <input type="text" id="wallet-address" readonly value="0x9B3a54D092f6B4b3d2eC676cd589f124E9921E71" placeholder="Your wallet address">
              <button class="copy-address-button">
                <i class="fas fa-copy"></i> Copy
              </button>
            </div>
        ...
        `;
        appContainer.appendChild(qrScreen);
      }
      
      // Fix back button on QR screen
    ...
    console.log('Fixed receive screen');
  }
  
  // Populate token list for receive screen
  function populateReceiveTokenList() {
    ...
    } else if (window.walletData && 
               window.walletData[activeWallet] && 
               window.walletData[activeWallet].tokens) {
      tokens = window.walletData[activeWallet].tokens;
    } else {
      // Fallback tokens if no wallet data
      tokens = [
        {
          id: 'btc',
          name: 'Bitcoin',
    ...
    tokens.forEach(token => {
      ...
      // Safely handle chain badge (fix the 404 error)
      let chainBadgeHtml = '';
      if (token.chainBadge && token.chainBadge.startsWith('http')) {
        chainBadgeHtml = `<div class="chain-badge"><img src="${token.chainBadge}" alt="Network"></div>`;
      } else if (token.id === 'usdt' || token.id === 'twt' || token.id === 'bnb') {
        chainBadgeHtml = `<div class="chain-badge"><img src="https://cryptologos.cc/logos/bnb-bnb-logo.png" alt="Network"></div>`;
      }
      
      // Set token content
      tokenItem.innerHTML = `
        <div class="token-icon">
          <img src="${token.icon || getTokenLogoUrl(token.id)}" alt="${token.name}">
          ${chainBadgeHtml}
        </div>
        <div class="token-info">
          <div class="token-name">${token.symbol}</div>
          <div class="token-price">${token.name}</div>
        </div>
      ...
      tokenItem.addEventListener('click', function() {
        const qrScreen = document.getElementById('receive-qr-screen');
        const receiveScreen = document.getElementById('receive-screen');
        
        if (qrScreen && receiveScreen) {
          // Update token symbol
          const tokenSymbol = qrScreen.querySelector('#receive-token-symbol');
          if (tokenSymbol) {
            tokenSymbol.textContent = token.symbol;
          }
          
          // Update QR image (in a real app, this would generate a unique QR)
          const qrImage = qrScreen.querySelector('#receive-qr-code');
          if (qrImage) {
            qrImage.src = token.icon || getTokenLogoUrl(token.id);
          }
          
          // Hide receive screen and show QR screen
    ...
    console.log('Populated receive token list');
  }
  
  // Function to get token logo URL (copy from code)
  ...
  function fixVisibilityToggle() {
    ...
    newToggle.addEventListener('click', function() {
      ...
      // Check if balance is currently hidden
      const isHidden = icon.classList.contains('fa-eye-slash');
      
      if (isHidden) {
        // Show balance - switch to eye icon
        ...
        if (window.cachedBalance) {
          ...
          // Fallback to calculating from active wallet
          const activeWallet = window.activeWallet || 'main';
          if (window.currentWalletData && window.currentWalletData[activeWallet]) {
            const totalBalance = window.currentWalletData[activeWallet].totalBalance || 0;
            balanceAmount.textContent = '$' + totalBalance.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          } else {
            balanceAmount.textContent = '$0.00';
          }
        }
        ...
        document.querySelectorAll('.token-balance').forEach(element => {
          const originalValue = element.getAttribute('data-original-balance');
          if (originalValue) {
            element.textContent = originalValue;
          }
        });
      } else {
        // Hide balance - switch to eye-slash icon
        ...
        document.querySelectorAll('.token-balance').forEach(element => {
          // Save original value
          element.setAttribute('data-original-balance', element.textContent);
          element.textContent = '••••••';
        });
      }
    });
    
    console.log('Fixed visibility toggle');
  }
  
  // Connect button events for the receive screen
  function connectReceiveEvents() {
    // Connect back button on receive screen
    const backButton = document.querySelector('#receive-screen .back-button');
    if (backButton) {
      backButton.addEventListener('click', function() {
    ...
    // Connect copy button on receive QR screen
    const copyButton = document.querySelector('#receive-qr-screen .copy-address-button');
    if (copyButton) {
  ...
  // Fix main wallet data initialization
  function fixWalletDataInitialization() {
    // Check if wallet data exists
    if (!window.walletData || !window.currentWalletData) {
      console.log('Initializing missing wallet data');
      ...
      for (const walletKey in window.walletData) {
        if (window.walletData.hasOwnProperty(walletKey)) {
          const wallet = window.walletData[walletKey];
      ...
      console.log('Wallet data initialized');
    }
  }
  
  // Fixed population of token list
  function fixPopulateTokenList() {
    // Check if we're replacing a broken function
    if (typeof window.populateTokenList === 'function') {
      console.log('Patching token list population function');
      
      // Save reference to original function (for debugging)
      window._originalPopulateTokenList = window.populateTokenList;
      
      // Replace with fixed version
      window.populateTokenList = function() {
        console.log('Using fixed token list population');
        ...
        if (totalBalanceElement) {
          const wallet = window.currentWalletData[activeWallet];
          if (wallet) {
            totalBalanceElement.textContent = '$' + wallet.totalBalance.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          }
        }
        
        console.log(`Token list populated with ${tokens.length} tokens`);
      };
    }
  }
  ...
  function applyAllFixes() {
    try {
      // Fix wallet data initialization
      fixWalletDataInitialization();
      
      // Fix token list population
      fixPopulateTokenList();
      
      // Fix header heights
      fixHeaderHeight();
      
      // Fix the receive screen
      fixReceiveScreen();
      
      // Fix visibility toggle
      fixVisibilityToggle();
      
      // Populate the receive screen
      populateReceiveTokenList();
      
      // Connect receive screen events
      connectReceiveEvents();
      
      // Make sure main token list is populated
      if (typeof window.populateTokenList === 'function') {
