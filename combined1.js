...
function connectEventHandlers() {
  ...
  try {
    ...
    if (window.currentWalletData && window.currentWalletData[activeWallet]) {
      // Update wallet data and UI
      window.updateWalletUI(activeWallet);
      
      // Update token selection list for send screen
      if (typeof window.tokenSelectionManager !== 'undefined' && 
          typeof window.tokenSelectionManager.populateTokenList === 'function') {
        window.tokenSelectionManager.populateTokenList();
      }
      
      // Populate main token list in wallet view
      const tokenList = document.getElementById('token-list');
      if (tokenList) {
        // Clear the token list first
        tokenList.innerHTML = '';
        
        // Get tokens from current wallet data
        const tokens = window.currentWalletData[activeWallet].tokens || [];
        ...
        tokens.forEach(token => {
          ...
          // Create token HTML structure
          tokenItem.innerHTML = `
            <div class="token-icon">
              <img src="${window.getTokenLogoUrl(token.id)}" alt="${token.name}">
              ${token.chainBadge ? `<div class="chain-badge"><img src="${token.chainBadge}" alt="Chain"></div>` : ''}
            </div>
            <div class="token-info">
        ...
        // Add event listeners to token items
        document.querySelectorAll('.token-item').forEach(item => {
      ...
      console.error('Wallet data not available for demo balance');
      return false;
    }
  } catch (error) {
    console.error('Error setting up demo balance:', error);
    return false;
  }
};
